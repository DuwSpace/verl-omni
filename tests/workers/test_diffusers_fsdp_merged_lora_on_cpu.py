...
def test_param_to_cpu_handles_tensor_and_offloaded_dtensor(monkeypatch):
    from verl_omni.utils.fsdp_utils import _param_to_cpu

    tensor = torch.randn(2, requires_grad=True)
    materialized = _param_to_cpu(tensor)
    assert materialized.device.type == "cpu"
    assert not materialized.requires_grad
    torch.testing.assert_close(materialized, tensor.detach())

    class FakeDTensor:
        device = torch.device("cpu")
        device_mesh = SimpleNamespace(device_type="npu")
    ...
    monkeypatch.setattr("verl.utils.device.get_device_id", lambda: torch.device("cpu"))
    fake = FakeDTensor()
    materialized = _param_to_cpu(fake)
    assert fake.moves == [(torch.device("cpu"), True)]
    assert materialized.device.type == "cpu"
    assert not materialized.requires_grad
    torch.testing.assert_close(materialized, torch.tensor([1.0, 2.0]))


@pytest.mark.parametrize("uses_cpu_offload_policy", [False, True])
def test_adapter_state_context_defers_device_moves_to_fsdp2_policy(monkeypatch, uses_cpu_offload_policy):
    import verl.utils.fsdp_utils as verl_fsdp_utils

    import verl_omni.utils.fsdp_utils as omni_fsdp_utils

    module = torch.nn.Linear(2, 2, bias=False)
    module.set_adapter = MagicMock()
    engine = object.__new__(PPODiffusersFSDPEngine)
    engine.module = module
    engine._is_offload_param = True
    engine._uses_fsdp2_cpu_offload_policy = uses_cpu_offload_policy

    load = MagicMock()
    offload = MagicMock()
    monkeypatch.setattr(verl_fsdp_utils, "fsdp_version", lambda _module: 2)
    monkeypatch.setattr(verl_fsdp_utils, "load_fsdp_model_to_gpu", load)
    monkeypatch.setattr(verl_fsdp_utils, "offload_fsdp_model_to_cpu", offload)
    monkeypatch.setattr(
        omni_fsdp_utils, "fsdp_summon_full_params", lambda _module, **_kwargs: contextmanager(lambda: (yield))()
    )
    monkeypatch.setattr("verl.utils.memory_utils.aggressive_empty_cache", MagicMock())

    with engine._adapter_state_context():
        pass

    module.set_adapter.assert_called_once_with("default")
    assert load.call_count == (0 if uses_cpu_offload_policy else 1)
    assert offload.call_count == (0 if uses_cpu_offload_policy else 1)
