...
"""Ascend NPU replay test for the batch-native OmniNFT CLAP reward."""
...
_MODEL_REVISION = "laion/clap-htsat-unfused@8fa0f1c6d0433df6e97c127f64b2a1d6c0dcda8a"
...
def _make_config(model_path):
    ...
    config.reward.component_order = ["clap"]
...
def test_real_clap_scores_complete_local_replay_batch():
    model_path = os.environ["CLAP_MODEL_PATH"]
    replay_path = os.environ["OMNIFT_REPLAY_PATH"]
    actor_cls = ray.remote(resources={"NPU": 1})(_ClapReplayActor)
    ...
    batch_metrics = batch_output["reward_extra_info"]["clap"]["metrics"]
    reference_metrics = reference_output["reward_extra_info"]["clap"]["metrics"]
    ...
    assert ((0 <= batch_output["rm_scores"]) & (batch_output["rm_scores"] <= 1)).all()
