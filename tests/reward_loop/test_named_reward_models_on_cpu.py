...
from verl_omni.reward_loop.reward_manager.multi import _validate_reward_aggregation
...
@pytest.mark.parametrize("aggregation", ["preserve_component", "sum", ""])
def test_unknown_reward_aggregation_fails_closed(aggregation):
    config = _config()
    config.reward.aggregation = aggregation

    with pytest.raises(ValueError, match="reward.aggregation"):
        _validate_reward_aggregation(config)


def test_preserve_components_requires_named_models():
    config = _config()
