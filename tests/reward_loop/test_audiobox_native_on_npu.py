...
"""Ascend NPU replay test for the batch-native OmniNFT AudioBox reward."""
...
_MODEL_REVISION = "facebook/audiobox-aesthetics@9b1dd8e5df9af7216e836a98974fe3b82c56ded6"
...
def _make_config(model_path):
    ...
    config.reward.component_order = ["audiobox"]
...
class _AudioBoxReplayActor:
    def __init__(self, model_path):
        torch.npu.set_device(0)
        manager = MultiModalRewardManager(_make_config(model_path), tokenizer=None, compute_score=None)
...
def test_real_audiobox_scores_complete_local_replay_batch():
    pytest.importorskip("audiobox_aesthetics")
    model_path = os.environ["AUDIOBOX_MODEL_PATH"]
    replay_path = os.environ["OMNIFT_REPLAY_PATH"]
    actor_cls = ray.remote(resources={"NPU": 1})(_AudioBoxReplayActor)
    actor = actor_cls.remote(model_path)
    try:
    ...
    batch_metrics = batch_output["reward_extra_info"]["audiobox"]["metrics"]
    reference_metrics = reference_output["reward_extra_info"]["audiobox"]["metrics"]
    ...
    assert batch_metrics["batch_size"] == reference_metrics["batch_size"] == 8
    ...
    assert batch_metrics["window_count"] == reference_metrics["window_count"] == 8
    assert torch.isfinite(batch_output["rm_scores"]).all()
    torch.testing.assert_close(batch_output["rm_scores"], reference_output["rm_scores"], rtol=1e-4, atol=1e-4)
