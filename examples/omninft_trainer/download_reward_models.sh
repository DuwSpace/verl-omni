...
if ! reference_head="$(git -C "${REFERENCE_ROOT}" rev-parse HEAD 2>/dev/null)"; then
  echo "error: ${REFERENCE_ROOT} is not a valid Git checkout" >&2
  exit 1
fi
if [[ "${reference_head}" != "${REFERENCE_REVISION}" ]]; then
  echo "error: ${REFERENCE_ROOT} must be pinned to ${REFERENCE_REVISION}, got ${reference_head}" >&2
  exit 1
fi
if ! reference_status="$(
  git -C "${REFERENCE_ROOT}" status --porcelain=v1 --untracked-files=all 2>/dev/null
)"; then
  echo "error: failed to inspect the OmniNFT reference checkout at ${REFERENCE_ROOT}" >&2
  exit 1
fi
if [[ -n "${reference_status}" ]]; then
  echo "error: ${REFERENCE_ROOT} must be a pristine checkout of ${REFERENCE_REVISION}" >&2
  echo "${reference_status}" >&2
  exit 1
fi
