...
class DiffusionNFTLoss(DiffusionLossFn):
    ...
    def _select_train_timesteps(
        ...
        Shuffle each row of ``train_timesteps`` and keep the first ``num_train``
        entries; ``seed`` controls the shuffle (deterministic when set).
        """
        ...
        # LTX-2.3 uses a shifted scheduler grid whose entries are generally
        # fractional.  Selecting entries must not quantize that grid: the
        # engine consumes the original scheduler value when constructing x_t.
        # Integer-based pipelines keep their dtype naturally.
...
class OmniNFTLoss(DiffusionNFTLoss):
    ...
    def compute_loss(
        ...
        *,
        ...
        """Combine the verified video and audio OmniNFT branch losses."""
    ...
    def prepare_actor_batch(batch: DataProto, reward_tensor: torch.Tensor, config: Any) -> DataProto:
        """Route component rewards into independent video/audio probabilities."""
        if "uid" not in batch.non_tensor_batch:
        ...
        batch.batch["timestep_modality_reward_probs"] = timestep_modality_reward_probs
        batch.batch["video_reward_prob"] = timestep_modality_reward_probs[..., 0]
        batch.batch["audio_reward_prob"] = timestep_modality_reward_probs[..., 1]
