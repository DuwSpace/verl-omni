        ...
        *,
        ...
        for name in reward_names:
            ...
            for modality in ("video", "audio"):
                if modality not in routing_weights:
                    raise ValueError(f"Reward '{name}' must define routing_weights.{modality}.")
                try:
                    value = float(routing_weights[modality])
                except (TypeError, ValueError) as exc:
                    raise ValueError(
                        f"Reward '{name}' routing weight for {modality} must be a finite non-negative number."
                    ) from exc
