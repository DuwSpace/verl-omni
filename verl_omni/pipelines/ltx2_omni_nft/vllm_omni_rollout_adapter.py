...
def _require_one_stage_recipe(pipeline_recipe: Any) -> None:
    ...
    if len(phases) != 1 or video_output_phase != 0 or audio_output_phase != 0:
        raise NotImplementedError(
            "LTX-2.3 OmniNFT requires one native denoising phase with video and audio emitted from phase 0."
        )
...
class LTX23OmniNFTPipeline(LTXTokenIdPromptMixin, LTX2Pipeline):
    ...
    def forward(
        ...
        **kwargs: Any,
        ...
        """Generate one batch and release captured native LTX state on exit."""
        self._clear_captured_state()
        try:
            return self._forward_impl(req, **kwargs)
        finally:
            # The returned CPU payload owns detached copies of every tensor it
            # needs.  Keeping these native contexts until the next request
            # otherwise retains the final NPU latent and prompt embeddings.
            self._clear_captured_state()
