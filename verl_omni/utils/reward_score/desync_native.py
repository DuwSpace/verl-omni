...
def _load_state(
    ...
    """Load the pinned Synchformer source and checkpoint."""
    for name, value in (
...
async def compute_score_batch(batch, reward_model, *, micro_batch_size: int, **kwargs) -> dict[str, Any]:
    ...
    return {
        ...
        "metrics": {
            ...
            "source_revision": _DEFAULT_SOURCE_REVISION,
        },
...
class DeSyncNativeModel:
    """Raw Synchformer inference adapter owned by the upstream native executor."""
    ...
    def close(self) -> None:
        """Release the complete model instance at worker shutdown."""
    ...
    def infer(self, video: torch.Tensor, audio: torch.Tensor) -> torch.Tensor:
        # Synchformer uses an attention path unsupported by the target NPU.
        # Scope the process-global compatibility switch to the forward instead
        # of coupling it to the executor-owned model lifecycle.
