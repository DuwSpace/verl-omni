...
class OmniNFTDiffusersFSDPEngine(NFTDiffusersFSDPEngine):
    ...
    def forward_backward_batch(
        ...
        """Reject single-latent timestep staging, which cannot represent joint AV inputs."""
        if tu.get_non_tensor_data(data, "enable_timestep_staging", default=False) and not forward_only:
            raise NotImplementedError(
                "OmniNFT does not support actor.enable_timestep_staging; keep it false for joint video/audio inputs."
            )
